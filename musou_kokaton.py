...
class Beam(pg.sprite.Sprite):
    ...
    ビームに関するクラス
    """
    def __init__(self, bird: Bird, add_angle: int = 0):
        """
        ビーム画像Surfaceを生成する
        引数 bird：ビームを放つこうかとん
        """
...
class NeoBeam:
    ...
    def gen_beams(bird: Bird, num: int) -> list[Beam]:
        """
        指定された数のビームを-50度から+50度の範囲で等間隔に生成し、リストとして返す
        """
        # numが1の場合のゼロ除算を避ける
        if num == 1:
            return [Beam(bird, add_angle=0)]
        
        beams = []
        angle_step = 100 / (num - 1)  # 角度のステップを計算
        for i in range(num):
            angle = -50 + i * angle_step
            beams.append(Beam(bird, add_angle = int(angle)))
        return beams
...
class Gravity(pg.sprite.Sprite):
    """
    重力場に関するクラス（スライドの指示に基づく）
    画面全体を覆い、領域内の敵機と爆弾をせん滅させる
    """
    def __init__(self, life: int):
        ...
        super().__init__()
        self.life = life
        self.image = pg.Surface((WIDTH, HEIGHT)) # 画面全体
        self.color = (0, 0, 0)  # 黒
        self.image.fill(self.color)
        self.image.set_alpha(128)  # 半透明
        self.rect = self.image.get_rect()
        self.rect.center = (WIDTH//2, HEIGHT//2) # 画面中央に

    def update(self):
        """
        重力場の持続時間を減算し、0未満になったら消滅させる
        """
        self.life -= 1
        if self.life < 0:
            self.kill()
    ...
    while True:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE:
                if key_lst[pg.K_z]:
                    beams.add(NeoBeam.gen_beams(bird, 5))
                else:
                    beams.add(Beam(bird))
